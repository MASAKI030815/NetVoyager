...
def ping_internet_v6():
    for host, name in config.pingv6_targets:
        response_ping_internet_v6 = []
        short_packet_cmd = ["ping"] + config.pingv6_short_option + [host]
        ...
        large_packet_cmd = ["ping"] + config.pingv6_large_option + [host]
        ...
        response_ping_internet_v6 = f"{status_color}{status}\033[0m ({short_color}Short\033[0m / {large_color}Large\033[0m) : {host} ({name})"
        print(response_ping_internet_v6)